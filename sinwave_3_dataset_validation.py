...
drawinput = (random_sample.cpu()).numpy()
drawoutput = (final_pred.cpu()).detach().numpy()
print(drawinput)
print(drawoutput)

plt.plot(numbers, drawinput, label = 'input', color = 'blue', linestyle = "-")

plt.plot(numbers, drawoutput, label = 'output', color = 'red', linestyle = "-")
plt.grid(True)
plt.show()
